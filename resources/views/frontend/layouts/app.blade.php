...
<body class="{{ $showBrandIntro ? 'brand-intro-lock ' : '' }}frontend-theme bg-background-dark text-white antialiased overflow-x-hidden">
    ...
    @if(!empty($settings['whatsapp_number']))
    <a
        href="https://wa.me/{{ preg_replace('/[^0-9]/', '', $settings['whatsapp_number']) }}?text={{ urlencode('Halo Home Putra Interior, saya tertarik dengan layanan desain interior Anda.') }}"
        target="_blank"
        rel="noopener noreferrer"
        class="theme-keep-dark fixed bottom-6 right-6 z-[1000] flex h-[60px] w-[60px] items-center justify-center rounded-full bg-[#25D366] text-white shadow-lg shadow-green-500/30 transition-transform duration-200 hover:scale-105"
