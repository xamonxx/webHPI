...
@section('content')
@php
    $initialPhotos = $portfolio->photos->map(fn ($photo) => [
        'id' => $photo->id,
        'url' => $portfolio->resolveImageUrl($photo->path),
        'path' => $photo->path,
    ])->values();
@endphp

<form action="{{ route('admin.portfolio.update', $portfolio->id) }}" method="POST" enctype="multipart/form-data" data-portfolio-form>
    @csrf
    @method('PUT')
        ...
        <div class="space-y-6">
            <section class="glass-card relative z-20 p-5 sm:p-6 rounded-2xl border border-white/5">
                ...
                <div class="space-y-5">
                    <div>
                        ...
                        <input type="text" name="title" value="{{ old('title', $portfolio->title) }}" class="w-full bg-black/20 border border-white/10 rounded-xl px-4 py-3 text-white placeholder-gray-600 focus:outline-none focus:border-primary/50 focus:bg-black/40 transition-all font-medium" placeholder="Judul proyek" required>
                    ...
                    <div>
                        ...
                        <textarea name="description" rows="6" class="w-full bg-black/20 border border-white/10 rounded-xl px-4 py-3 text-white placeholder-gray-600 focus:outline-none focus:border-primary/50 focus:bg-black/40 transition-all" placeholder="Jelaskan detail project...">{{ old('description', $portfolio->description) }}</textarea>
            ...
            <section class="glass-card p-5 sm:p-6 rounded-2xl border border-white/5">
                ...
                <div class="grid grid-cols-1 lg:grid-cols-2 gap-6">
                    @include('admin.portfolio.partials.category-combobox', ['selectedCategory' => $portfolio->category])
                    ...
                    <div>
                        ...
                        <div class="flex flex-col sm:flex-row sm:items-center gap-4">
                            <label class="relative inline-flex items-center cursor-pointer">
                                <input type="checkbox" name="is_active" value="1" class="sr-only peer" @checked(old('is_active', $portfolio->is_active))>
                            ...
                            <label class="relative inline-flex items-center cursor-pointer">
                                <input type="checkbox" name="is_featured" value="1" class="sr-only peer" @checked(old('is_featured', $portfolio->is_featured))>
            ...
            <section class="glass-card relative z-0 p-5 sm:p-6 rounded-2xl border border-white/5">
                <div class="flex items-start justify-between gap-4 mb-5">
                    <div>
                        ...
                        <p class="mt-2 text-xs text-gray-500 leading-relaxed">Pertahankan, hapus, atau tambah foto sampai maksimal 5 foto.</p>
                    </div>
                ...
                <label id="photo-upload-zone" for="photos-input" class="group flex min-h-[180px] cursor-pointer flex-col items-center justify-center rounded-2xl border-2 border-dashed border-white/10 bg-black/20 p-6 text-center transition hover:border-primary/60 hover:bg-primary/5">
                    ...
                    <span class="text-sm font-bold text-white">Tambah Foto</span>
                ...
                <div class="mt-5 rounded-xl border border-yellow-500/20 bg-yellow-500/10 p-3 text-xs leading-relaxed text-yellow-300/80">
                    Foto yang dihapus baru benar-benar dihapus setelah perubahan disimpan.
                </div>
                ...
                <div class="mt-6 border-t border-white/10 pt-5">
                    <div class="relative z-10 flex flex-col sm:flex-row xl:flex-col gap-3">
                        ...
                        <button type="submit" data-submit-button class="inline-flex min-h-[52px] items-center justify-center gap-2 rounded-xl bg-linear-to-r from-primary to-yellow-600 px-5 font-bold uppercase tracking-wider text-black transition hover:shadow-lg hover:shadow-primary/20">
                            ...
                            <span data-submit-label>Simpan Perubahan</span>
                        </button>
                    </div>
                </div>
            </section>
...
@push('scripts')
<script>
    window.portfolioInitialPhotos = @json($initialPhotos);
</script>
