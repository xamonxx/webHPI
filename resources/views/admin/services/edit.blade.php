...
@section('page-title', 'Layanan / Edit Data')
        ...
        <div class="lg:col-span-2 space-y-6">
            ...
            <div class="glass-card p-6 rounded-2xl border border-white/5">
                <div class="flex items-center justify-between mb-6">
                    ...
                    <span class="text-xs text-gray-500 font-mono">ID: #{{ $service->id }}</span>
                </div>
                ...
                <div class="space-y-6">
                    ...
                    <div class="group">
                        ...
                        <textarea name="description" rows="5"
                            ...
                            placeholder="Jelaskan detail layanan..." required>{{ old('description', $service->description) }}</textarea>
                ...
                <div class="space-y-6">
                    ...
                    <div>
                        ...
                        <div class="flex items-center gap-3 p-4 bg-black/20 border border-white/10 rounded-xl mb-4">
                            <div class="w-14 h-14 bg-green-500/10 rounded-xl flex items-center justify-center border border-green-500/20">
                                <span class="material-symbols-outlined text-green-400 text-3xl" id="icon-preview">{{ old('icon', $service->icon) ?? 'home' }}</span>
                            </div>
                            <div>
                                <p class="text-white font-bold text-sm">Ikon Terpilih</p>
                                <p class="text-gray-500 text-xs font-mono" id="icon-name-display">{{ old('icon', $service->icon) ?? 'home' }}</p>
                            </div>
                        </div>
                        ...
                        <div class="grid grid-cols-6 gap-2 p-3 bg-black/20 border border-white/10 rounded-xl max-h-64 overflow-y-auto custom-scrollbar">
                            ...
                            <button type="button"
                                ...
                                data-icon="{{ $iconName }}"
                                title="{{ $icon['label'] }}">
                                <span class="material-symbols-outlined text-gray-400 group-hover:text-green-400 text-xl transition-colors">{{ $iconName }}</span>
                            </button>
                            @endforeach
                        </div>
                    ...
                    <div>
                        ...
                        <label class="relative inline-flex items-center cursor-pointer mt-1">
                            <input type="checkbox" name="is_active" value="1" class="sr-only peer" {{ old('is_active', $service->is_active ?? false) ? 'checked' : '' }}>
